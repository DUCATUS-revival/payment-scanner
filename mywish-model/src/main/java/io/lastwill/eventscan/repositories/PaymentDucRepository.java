...
public interface PaymentDucRepository extends CrudRepository<PaymentDetailsDUC, Long> {
    ...
    @Modifying
    @Transactional
    @Query("update PaymentDetailsDUC paymentDetailsDUC set paymentDetailsDUC.rxAmount = :rxAmount where paymentDetailsDUC.rxAddress = :rxAddress")
    void updateRxAmount(@Param("rxAddress") String rxAddress, @Param("rxAmount") BigInteger rxAmount);
